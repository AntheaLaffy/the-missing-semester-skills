...
主线：regex 是描述字符串集合的语言，用于模式匹配/校验/解析——命令行（grep/sed 用法见 `shell-data-wrangling`）、IDE、代码里都有。先学本讲基础，之后按需查参考资料，别背整门语言；用 regex101 开发调试。

## 示例

- `abc` 字面量；`missing|semester` 或；`\d{4}-\d{2}-\d{2}` YYYY-MM-DD 形状（不验日期合法性，"2026-01-99" 也匹配）；`.+@.+` 最弱的 email 形状。
...
- 提取：`re.match(r"\d{4}-(\d{2})-\d{2}", "2026-01-14").group(1)` → `'01'`。
- 替换里引用分组语法随工具不同：VS Code 用 `$1`/`$2`，Vim 用 `\1`/`\2`。
