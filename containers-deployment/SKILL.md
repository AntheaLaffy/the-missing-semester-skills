...
- Dockerfile 反例 → 正例：整版 python 镜像 → slim 变体；多个独立 RUN → 合并一条 `&&`；不锁版本；不清理 apt 缓存（`rm -rf /var/lib/apt/lists/*`）；以 root 运行；把 secret 打进层。正例：`FROM python:3.12-slim`、`apt-get install -y --no-install-recommends`、按 uv.lock 安装、最后 `COPY . /app`。差距是实打实的（示例：不优化 2GB → 优化后 0.5GB）。
- Dockerfile 是手写的，没有自动生成：从「应用要跑起来需要什么」倒推——有 C 扩展才装 GCC，用 CUDA 就选带 CUDA 的基底镜像（如 CuPy 场景）。
...
- 生产自启：systemd 管 compose（`Type=oneshot`、`ExecStart=docker compose up -d`、`WantedBy=multi-user.target`）——开机即起，`systemctl start/stop/status` 标准控制。systemd 的声明式语法选项繁杂，不熟时问 LLM 学选项很高效。
- Kubernetes：要跨多机扩容、故障转移、高可用时再上；学习曲线陡、运维开销大——小项目是过度设计。

## 部署与发布

- 部署 Web 服务：域名注册、DNS 指向你的服务器、nginx 反代处理 HTTPS 与路由。
- 发布镜像：先 `docker login` 向 registry（Docker Hub、ghcr.io 等）认证；镜像名要带你的用户名/组织前缀才有权限推（`ghcr.io/<user>/...`）——推上去后任何人都能 pull。
